from .utils import * # ~ will import accuracy(), models from "model_ops", ...
...
class SyncReplicasMaster_NN(NN_Trainer):
    ...
    def start(self):
        ...
        for i in range(self._checkpoint_step+1, self._max_steps+1):
            ...
            logger.info("PS_BASELINE: Master node is entering step: {}".format(i))
    ...
    def init_model_shapes(self):
        ...
        for param_idx, param in enumerate(self.network.parameters()):
            ...
            if self._update_mode in ("normal", "bulyan", "multi-krum", "coord-median", "sign-sgd"):
                self._grad_aggregate_buffer.append(np.zeros(param.size(), dtype=float_type))
        ...
        if self._update_mode in ("bulyan", "multi-krum", "coord-median", "sign-sgd"):
            self._robust_aggr_buffer = np.zeros((self.num_workers, self._model_param_counter), dtype=float_type)
    ...
    def async_bcast_step(self):
        ...
        for i in range(len(req_list)):
            ...
            # ~ test
            # print("DEBUG_M_BASELINE: PS waiting at step ", self.cur_step, " for ", i+1)
    ...
    def async_bcast_layer_weights_async(self):
        ...
        for layer_idx, layer in enumerate(self.network.parameters()):
            request_workers = []
            layer_to_send = layer.data.numpy().astype(float_type)
            for i in range(self.world_size):
                if i != 0:
    ...
    def async_bcast_layer_weights_bcast(self):
        ...
        for layer_idx, layer in enumerate(self.network.parameters()):
            ...
            # np.save('BASELINE_master_model_layer_'+str(layer_idx), layer_to_send)
            
            # try to see if collective communication is better here:
            msg_send = w_compress(layer_to_send)
            self.comm.bcast(msg_send, root=0)

    # ~ this will need to change if you do more than K gradient transmissions per step
    def async_fetch_gradient_start(self):
        ...
        for layer_idx, layer in enumerate(self.network.parameters()):
            for k in range(self._num_grad_to_collect): # ~ for each worker
                if self._compress_grad == 'compress':
                    req = self.comm.irecv(self.grad_accumulator.gradient_aggregator[layer_idx][k], source=k+1, tag=88+layer_idx)
                else:
                    # test
                    # logger.info("DEBUG_PS_BASELINE: layer_idx, len(self.grad_accumulator.gradient_aggregator): {} {}".format(layer_idx, len(self.grad_accumulator.gradient_aggregator)))
                    # logger.info("DEBUG_PS_BASELINE: k, len(self.grad_accumulator.gradient_aggregator[layer_idx]): {} {}".format(k, len(self.grad_accumulator.gradient_aggregator[layer_idx])))
    ...
    def _evaluate_model(self, validation_loader):
        ...
        logger.info('PS_BASELINE: Testset Performance: Cur Step:{} Prec@1: {} Prec@5: {}'.format(self.cur_step, prec1.numpy()[0], prec5.numpy()[0]))
    ...
    def _get_geo_median(self):
        ...
        logger.info("PS_BASELINE: Master Step: {} Found Geo Median Cost: {:.4f}".format(self.cur_step, time.time()-geo_median_start))
