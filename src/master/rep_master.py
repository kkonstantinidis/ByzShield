...
class CodedMaster(SyncReplicasMaster_NN):
    ...
    def init_model_shapes(self): # ~ main difference compared to draco_lite_master is that the list "self._coded_grads_buffer" contains flattened parameters instead of having the layer's shape
        tmp_aggregate_buffer = [] # ~ never used
        self._model_param_counter = 0
        for param_idx, param in enumerate(self.network.parameters()):
            shape = param.size()
            num_params = reduce((lambda x, y: x * y), shape)

            temp_tuple = (self._model_param_counter, self._model_param_counter+num_params) # ~ tuple with the range of indices for current layer's "flattened" parameters
            self._model_param_counter += num_params
            self._model_shape_pointer.append(temp_tuple) # ~ list (one element per model layer) of tuples ... (see above)       

            self._model_shapes.append(shape)
            self._grad_aggregate_buffer.append(np.zeros(shape, dtype=np.float32))
            tmp_aggregate_buffer.append(np.zeros(shape)) # ~ never used
        ...
        if self._update_mode == "normal":
            ...
            for k, v in self._group_list.items():
                if source in v:
                    ...
                    if len(_shape) == 1: # ~ if it's already 1 dimension just store it
                        self._coded_grads_buffer[k][v.index(source)][indices[0]:indices[1]] = gradient
                    elif len(_shape) > 1: # ~ otherwise, first flatten it ...
    ...
    def _grad_majority_vote(self):
        ...
        # ~ test
        # logger.info("DEBUG_PS_REP: Master grad value for a layer: {}".format(self._grad_aggregate_buffer[0]))
