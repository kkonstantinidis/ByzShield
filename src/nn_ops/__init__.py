...
# ~ returns the top-1 up to topk[-1] accuracies of a model
...
def accuracy(output, target, topk=(1,)):
    ...
    # returns the k largest elements and their indices of the given input tensor along a given dimension
    ...
    # computes element-wise equality, returns element-wise True-False
    correct = pred.eq(target.view(1, -1).expand_as(pred))

    # ~ compute all accuracies 1...k
    res = []
    for k in topk:
        correct_k = correct[:k].view(-1).float().sum(0, keepdim=True) # ~ count of all samples for which a True is in their top-k predictions
        res.append(correct_k.mul_(100.0 / batch_size)) # ~ compute percentage
    return res
...
class NN_Trainer(object):
    ...
    def validate(self, test_loader):
        ...
        for data, y_batch in test_loader:
            ...
            test_loss += F.nll_loss(output, target, size_average=False).data[0] # ~ sum up batch loss, Negative Log-Likelihood loss
