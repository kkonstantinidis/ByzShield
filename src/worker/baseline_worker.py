...
class DistributedWorker(NN_Trainer):
    ...
    def train(self, train_loader, test_loader):
        ...
        logger.info("W_BASELINE: Worker {}: starting training".format(self.rank))
        ...
        for num_epoch in range(self.max_epochs):
            for batch_idx, (train_image_batch, train_label_batch) in enumerate(train_loader):
                ...
                while True:
                    ...
                    first = False
                    logger.info("W_BASELINE: Rank of this node: {}, Current step: {}".format(self.rank, self.cur_step))
    ...
    def async_fetch_weights_bcast(self):
        layers_to_update = [] # ~ never used
        weights_to_update = []
        for layer_idx, layer in enumerate(self.model_recv_buf.recv_buf):
            if self.model_recv_buf.layer_cur_step[layer_idx] < self.cur_step: # ~ layer not yet fetched from master for current step, this "if" is probably useless since we do a synchronous bcast() unlike async_fetch_weights_async()
                layers_to_update.append(layer_idx) # ~ never used
    ...
    def _evaluate_model(self, test_loader):
        ...
        logger.info('W_BASELINE: Test set: Step: {}, Average loss: {:.4f}, Prec@1: {} Prec@5: {}'.format(self.cur_step, test_loss, prec1, prec5))
    ...
    def _load_model(self, file_path):
        ...
        logger.info("W_BASELINE: Validation Worker Done Loading Checkpoint from {}".format(file_path))