...
```
**Note**: the above command won't copy the data sets to the workers. Those will be fetched from all machines from the EFS folder. For this to work, the EFS folder and the project folder `BYZSHIELD` need to be in the same directory. Since the project is stored by `local_script.sh` in `~/BYZSHIELD` which is `/home/ubuntu/shared` for AWS Ubuntu isntances, then you need to set the EFS folder to be `~/shared` or `/home/ubuntu/shared`. 
...
| `mode` | Robust aggregation method: `coord-median`, `bulyan`, `multi-krum`, `sign-sgd` or `geometric_median` (only supported in vanilla). |
